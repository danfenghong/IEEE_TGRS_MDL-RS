...
def mynetwork(x1, x2, parameters, isTraining, momentums = 0.9, epsilons = 1e-3):
    ...
    with tf.name_scope("joint_encoder_layer"):
         ...
         joint_encoder_layer_z3 = tf.add(tf.matmul(joint_encoder_layer_a2, parameters['joint_encoder_h3']), parameters['joint_encoder_b3'])
...
def train_mynetwork(x1_train_set, x2_train_set, x1_test_set, x2_test_set, y_train_set, y_test_set, MODEL,
    ...
    with tf.Session() as sess:
        ...
        #M = sess.run(tf.convert_to_tensor(tf.confusion_matrix(tf.argmax(yT),tf.argmax(joint_layerT))))
        #print(confuse_martix)

        print("save model")
        save_path = saver.save(sess,"D:\Python_Project\MDL-RS/save_Middle_FC/model.ckpt")
