...
def initialize_parameters():
    ...
    x1_encoder_h5 = tf.get_variable("x1_encoder_h5", [128,15], initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    x1_encoder_b5 = tf.get_variable("x1_encoder_b5", [15], initializer = tf.zeros_initializer())   
...
def mynetwork(x1, parameters, isTraining):
    ...
    l2_loss =  tf.nn.l2_loss(parameters['x1_encoder_h1']) + tf.nn.l2_loss(parameters['x1_encoder_h2'])\
               + tf.nn.l2_loss(parameters['x1_encoder_h3']) + tf.nn.l2_loss(parameters['x1_encoder_h4']) + tf.nn.l2_loss(parameters['x1_encoder_h5'])
    
    return x1_encoder_layer_z5, l2_loss
...
def train_mynetwork(x1_train_set, x1_test_set, y_train_set, y_test_set,
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        save_path = saver.save(sess,"D:\Python_Project\MDL-RS/save_HSI/model.ckpt")
