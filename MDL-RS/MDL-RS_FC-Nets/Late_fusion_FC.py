...
def initialize_parameters():
    ...
    x2_encoder_h5 = tf.get_variable("x2_encoder_h5", [128,128], initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    x2_encoder_b5 = tf.get_variable("x2_encoder_b5", [128], initializer = tf.zeros_initializer())
    ...
    x2_encoder_h6 = tf.get_variable("x2_encoder_h6", [128,64], initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    x2_encoder_b6 = tf.get_variable("x2_encoder_b6", [64], initializer = tf.zeros_initializer())
      
    joint_encoder_h1 = tf.get_variable("joint_encoder_h1", [128,15], initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    joint_encoder_b1 = tf.get_variable("joint_encoder_b1", [15], initializer = tf.zeros_initializer())       
    ...
    parameters = {"x1_encoder_h1": x1_encoder_h1,
                  ...
                  "x2_encoder_h6": x2_encoder_h6,
                  "x2_encoder_b6": x2_encoder_b6, 
...
def mynetwork(x1, x2, parameters, isTraining, momentums = 0.9, epsilons = 1e-3):
    ...
    with tf.name_scope("encoder_layer_5"):
         ...
         x1_encoder_layer_z5 = tf.matmul(x1_encoder_layer_a4, parameters['x1_encoder_h5']) + parameters['x1_encoder_b5']
         x1_encoder_layer_z5_bn = tf.layers.batch_normalization(x1_encoder_layer_z5, momentum = momentums, epsilon = epsilons, training = isTraining)                                             
         x1_encoder_layer_a5 = tf.nn.relu(x1_encoder_layer_z5_bn)
         
         x2_encoder_layer_z5 = tf.matmul(x2_encoder_layer_a4, parameters['x2_encoder_h5']) + parameters['x2_encoder_b5']
         x2_encoder_layer_z5_bn = tf.layers.batch_normalization(x2_encoder_layer_z5, momentum = momentums, epsilon = epsilons, training = isTraining)                                               
         x2_encoder_layer_a5 = tf.nn.relu(x2_encoder_layer_z5_bn)


    with tf.name_scope("encoder_layer_6"):
         
         x1_encoder_layer_z6 = tf.matmul(x1_encoder_layer_a5, parameters['x1_encoder_h6']) + parameters['x1_encoder_b6']
         x1_encoder_layer_z6_bn = tf.layers.batch_normalization(x1_encoder_layer_z6, momentum = momentums, epsilon = epsilons, training = isTraining)                                             
         x1_encoder_layer_a6 = tf.nn.relu(x1_encoder_layer_z6_bn)
         
         x2_encoder_layer_z6 = tf.matmul(x2_encoder_layer_a5, parameters['x2_encoder_h6']) + parameters['x2_encoder_b6']
         x2_encoder_layer_z6_bn = tf.layers.batch_normalization(x2_encoder_layer_z6, momentum = momentums, epsilon = epsilons, training = isTraining)                                               
         x2_encoder_layer_a6 = tf.nn.relu(x2_encoder_layer_z6_bn)
         
    # joint layer
    joint_encoder_layer = tf.concat([x1_encoder_layer_a6, x2_encoder_layer_a6], 1)
    ...
    l2_loss =   tf.nn.l2_loss(parameters['joint_encoder_h1'])\
                ...
                + tf.nn.l2_loss(parameters['x1_encoder_h3']) + tf.nn.l2_loss(parameters['x2_encoder_h3']) + tf.nn.l2_loss(parameters['x2_encoder_h4']) + tf.nn.l2_loss(parameters['x1_encoder_h4'])\
                + tf.nn.l2_loss(parameters['x1_encoder_h5']) + tf.nn.l2_loss(parameters['x2_encoder_h5']) + tf.nn.l2_loss(parameters['x1_encoder_h6']) + tf.nn.l2_loss(parameters['x2_encoder_h6'])
               
    return joint_encoder_layer_z1, l2_loss
...
def train_mynetwork(x1_train_set, x2_train_set, x1_test_set, x2_test_set, y_train_set, y_test_set, MODEL,
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        save_path = saver.save(sess,"D:\Python_Project\MDL-RS/save_Late_FC/model.ckpt")
