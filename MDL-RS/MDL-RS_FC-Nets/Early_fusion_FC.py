...
def initialize_parameters():
    ...
    x1_encoder_h1 = tf.get_variable("x1_encoder_h1", [165,16], initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    ...
    x1_encoder_h5 = tf.get_variable("x1_encoder_h5", [128,128], initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    x1_encoder_b5 = tf.get_variable("x1_encoder_b5", [128], initializer = tf.zeros_initializer())
    x1_encoder_h6 = tf.get_variable("x1_encoder_h6", [128,64], initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    x1_encoder_b6 = tf.get_variable("x1_encoder_b6", [64], initializer = tf.zeros_initializer())
    x1_encoder_h7 = tf.get_variable("x1_encoder_h7", [64,15], initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    x1_encoder_b7 = tf.get_variable("x1_encoder_b7", [15], initializer = tf.zeros_initializer()) 
    ...
    parameters = {"x1_encoder_h1": x1_encoder_h1,
                  ...
                  "x1_encoder_h5": x1_encoder_h5,
                  "x1_encoder_b5": x1_encoder_b5,
                  "x1_encoder_h6": x1_encoder_h6,
                  "x1_encoder_b6": x1_encoder_b6,
                  "x1_encoder_h7": x1_encoder_h7,
                  "x1_encoder_b7": x1_encoder_b7}
...
def mynetwork(x1, parameters, isTraining):
    ...
    with tf.name_scope("encoder_layer_1"):
         ...
         x1_encoder_layer_z1_bn = tf.layers.batch_normalization(x1_encoder_layer_z1, momentum = 0.9, training = isTraining)
    ...
    with tf.name_scope("encoder_layer_2"):
         ...
         x1_encoder_layer_z2_bn = tf.layers.batch_normalization(x1_encoder_layer_z2, momentum = 0.9, training = isTraining)                                             
    ...
    with tf.name_scope("encoder_layer_3"):
         ...
         x1_encoder_layer_z3_bn = tf.layers.batch_normalization(x1_encoder_layer_z3, momentum = 0.9, training = isTraining)                                             
    ...
    with tf.name_scope("encoder_layer_4"):
         ...
         x1_encoder_layer_z4_bn = tf.layers.batch_normalization(x1_encoder_layer_z4, momentum = 0.9, training = isTraining)                                             
         x1_encoder_layer_a4 = tf.nn.relu(x1_encoder_layer_z4_bn)
         
    with tf.name_scope("joint_encoder_layer"):
         
         joint_encoder_layer_z1 = tf.matmul(x1_encoder_layer_a4, parameters['x1_encoder_h5']) + parameters['x1_encoder_b5']                                          
         joint_encoder_layer_z1_bn = tf.layers.batch_normalization(joint_encoder_layer_z1, momentum = 0.9, training = isTraining)  
         joint_encoder_layer_a1 = tf.nn.relu(joint_encoder_layer_z1_bn)
         
         joint_encoder_layer_z2 = tf.matmul(joint_encoder_layer_a1, parameters['x1_encoder_h6']) + parameters['x1_encoder_b6']
         joint_encoder_layer_z2_bn = tf.layers.batch_normalization(joint_encoder_layer_z2, momentum = 0.9, training = isTraining)
         joint_encoder_layer_a2 = tf.nn.relu(joint_encoder_layer_z2_bn)
         
         joint_encoder_layer_z3 = tf.add(tf.matmul(joint_encoder_layer_a2, parameters['x1_encoder_h7']), parameters['x1_encoder_b7'])

    l2_loss =  tf.nn.l2_loss(parameters['x1_encoder_h5']) + tf.nn.l2_loss(parameters['x1_encoder_h6']) + tf.nn.l2_loss(parameters['x1_encoder_h7'])\
               + tf.nn.l2_loss(parameters['x1_encoder_h1']) + tf.nn.l2_loss(parameters['x1_encoder_h2'])\
               + tf.nn.l2_loss(parameters['x1_encoder_h3']) + tf.nn.l2_loss(parameters['x1_encoder_h4'])
    
    return joint_encoder_layer_z3, l2_loss
...
def train_mynetwork(x1_train_set, x1_test_set, y_train_set, y_test_set, MODEL,
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs + 1):
            ...
            if MODEL.strip() == 'CML':  
               ...
               feature, epoch_cost_dev, epoch_acc_dev = sess.run([joint_layerT, cost, accuracy], feed_dict={x1: x1_test_set, y: y_test_set, isTraining: True})
        ...
        print("save model")
        save_path = saver.save(sess,"D:\Python_Project\MDL-RS/save_Early_FC/model.ckpt")
...
if MODEL.strip() == 'MML':
   parameters, val_acc, feature = train_mynetwork(np.concatenate((HSI_TrSet, LiDAR_TrSet), 1), np.concatenate((HSI_TeSet, LiDAR_TeSet), 1), Y_train, Y_test, 'MML')

if MODEL.strip() == 'CML-LiDAR':
   parameters, val_acc, feature = train_mynetwork(np.concatenate((HSI_TrSet, LiDAR_TrSet), 1), np.concatenate((np.zeros_like(HSI_TeSet), LiDAR_TeSet), 1), Y_train, Y_test, 'CML')

if MODEL.strip() == 'CML-HSI':
   parameters, val_acc, feature = train_mynetwork(np.concatenate((HSI_TrSet, LiDAR_TrSet), 1), np.concatenate((HSI_TeSet, np.zeros_like(LiDAR_TeSet)), 1), Y_train, Y_test, 'CML')
