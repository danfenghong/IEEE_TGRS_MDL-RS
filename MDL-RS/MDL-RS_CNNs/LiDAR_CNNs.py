...
def initialize_parameters():
    ...
    x1_conv_w1 = tf.get_variable("x1_conv_w1", [3,3,21,16], dtype=tf.float32, initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
...
def mynetwork(x1, parameters, isTraining):
    ...
    x1 = tf.reshape(x1, [-1, 7, 7, 21], name = "x1")
    ...
    with tf.name_scope("encoder_layer_5"):
         ...
         x1_conv_layer_z6 = tf.nn.conv2d(x1_conv_layer_a4_po, parameters['x1_conv_w5'], strides=[1, 1, 1, 1], padding='SAME') + parameters['x1_conv_b5'] 
...
def train_mynetwork(x1_train_set, x1_test_set, y_train_set, y_test_set,
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        save_path = saver.save(sess,"D:\Python_Project\MDL-RS/save_LiDAR_CNN/model.ckpt")
