...
def initialize_parameters():
    ...
    x1_conv_w4 = tf.get_variable("x1_conv_w4", [1,1,256,128], initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    ...
    x1_conv_w6 = tf.get_variable("x1_conv_w6", [1,1,64,15], initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
...
def mynetwork(x1, x2, parameters, isTraining):
    ...
    with tf.name_scope("encoder_layer_3"):
         ...
         joint_encoder_layer = tf.concat([x1_conv_layer_a31, x2_conv_layer_a31], 3)
    
    with tf.name_scope("encoder_layer_5"):

         x1_conv_layer_z4 = tf.nn.conv2d(joint_encoder_layer, parameters['x1_conv_w4'], strides=[1, 1, 1, 1], padding='SAME') + parameters['x1_conv_b4']   
    ...
    l2_loss =  tf.nn.l2_loss(parameters['x1_conv_w1']) + tf.nn.l2_loss(parameters['x1_conv_w2']) + tf.nn.l2_loss(parameters['x1_conv_w3']) + tf.nn.l2_loss(parameters['x1_conv_w4'])\
               + tf.nn.l2_loss(parameters['x2_conv_w1']) + tf.nn.l2_loss(parameters['x2_conv_w2']) + tf.nn.l2_loss(parameters['x2_conv_w3'])\
               + tf.nn.l2_loss(parameters['x1_conv_w5']) + tf.nn.l2_loss(parameters['x1_conv_w6']) + tf.nn.l2_loss(parameters['x1_conv_w31']) + tf.nn.l2_loss(parameters['x2_conv_w31'])
...
def train_mynetwork(x1_train_set, x2_train_set, x1_test_set, x2_test_set, y_train_set, y_test_set, MODEL,
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        save_path = saver.save(sess,"D:\Python_Project\MDL-RS/save_CNN_Middle/model.ckpt")
