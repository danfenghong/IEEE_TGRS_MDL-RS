...
def initialize_parameters():
    ...
    x1_conv_w1 = tf.get_variable("x1_conv_w1", [3,3,165,16], dtype=tf.float32, initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    ...
    x1_conv_w5 = tf.get_variable("x1_conv_w5", [1,1,128,128], dtype=tf.float32, initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    x1_conv_b5 = tf.get_variable("x1_conv_b5", [128], initializer = tf.zeros_initializer())   

    x1_conv_w6 = tf.get_variable("x1_conv_w6", [1,1,128,64], dtype=tf.float32, initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
    x1_conv_b6 = tf.get_variable("x1_conv_b6", [64], initializer = tf.zeros_initializer())    
 
    x1_conv_w7 = tf.get_variable("x1_conv_w7", [1,1,64,15], dtype=tf.float32, initializer = tf.contrib.layers.variance_scaling_initializer(seed = 1))
...
def mynetwork(x1, parameters, isTraining):
    ...
    x1 = tf.reshape(x1, [-1, 7, 7, 165], name = "x1")
    ...
    with tf.name_scope("encoder_layer_7"):
    ...
    l2_loss =  tf.nn.l2_loss(parameters['x1_conv_w1']) + tf.nn.l2_loss(parameters['x1_conv_w2']) + tf.nn.l2_loss(parameters['x1_conv_w3']) + tf.nn.l2_loss(parameters['x1_conv_w4'])\
               + tf.nn.l2_loss(parameters['x1_conv_w5']) + tf.nn.l2_loss(parameters['x1_conv_w6']) + tf.nn.l2_loss(parameters['x1_conv_w7'])
...
def train_mynetwork(x1_train_set, x1_test_set, y_train_set, y_test_set, MODEL,
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        save_path = saver.save(sess,"D:\Python_Project\MDL-RS/save_CNN_Early/model.ckpt")
